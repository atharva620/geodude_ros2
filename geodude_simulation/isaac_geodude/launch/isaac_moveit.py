...
# Get the Geodude USD file path
GEODUDE_USD_PATH = os.path.join(parent_dir, "geodude_latest/geodude.usd")

# CAMERA_PRIM_PATH = f"{GEODUDE_STAGE_PATH}/camera_mount_front_horizontal_link/geometry/realsense/realsense_camera"
...
# Loading the geodude robot USD
prims.create_prim(
    GEODUDE_STAGE_PATH,
    "Xform",
    position=np.array([0, -0.64, 0]),
    orientation=rotations.gf_rotation_to_np_array(Gf.Rotation(Gf.Vec3d(0, 0, 1), 90)),
    usd_path=GEODUDE_USD_PATH,
)
...
try:
    og_keys_set_values = [
        ...
        # Set the /geodude target prim to Articulation Controller node
        ("ArticulationController.inputs:robotPath", GEODUDE_STAGE_PATH),
...
# Setting the /geodude target prim to Publish JointState node
set_target_prims(
    primPath="/ActionGraph/PublishJointState", targetPrimPaths=[f"{GEODUDE_STAGE_PATH}"]
)
...
physics_context.set_physics_dt(1.0 / 300.0)
print(simulation_context.get_physics_dt())
